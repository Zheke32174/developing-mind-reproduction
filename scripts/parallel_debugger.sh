...
if [ -f "$ERROR_LOG" ]; then
    ...
    timeout 90s gemini-clean --yolo --skip-trust -p "The background evolution command '$FAILED_COMMAND' failed with the following error: $(cat "$ERROR_LOG"). Debug this issue, apply a patch to the substrate, and signal a restart."
